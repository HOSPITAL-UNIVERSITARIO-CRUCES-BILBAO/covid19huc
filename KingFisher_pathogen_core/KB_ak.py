...
def run(ctx: protocol_api.ProtocolContext):
    ...
    def move_vol_multichannel(pipet, reagent, source, dest, vol, air_gap_vol, x_offset,
        ...
        if touch_tip == True:
            pipet.touch_tip(dest.top(z = -2), v_offset = -5, radius = 0.9)
...
####################################
    ...
    WashBuffer2_100ul_plate1 = ctx.load_labware(
        'kf_96_wellplate_2400ul', '2', 'Wash Buffer 2 Deepwell plate 1')

    # Lysis buffer 100ul Deepwell plate
    ############################################
    Lysisbuffer_100ul_plate1 = ctx.load_labware(
        'kf_96_wellplate_2400ul', '6', 'Lysis buffer Deepwell plate 1')
